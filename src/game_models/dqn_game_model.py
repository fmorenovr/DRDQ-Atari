...
from convolutional_networks.neural_networks import ConvolutionalNeuralNetwork
...
class DQNGameModel(BaseGameModel):
  def __init__(self, game_name, mode_name, input_shape, action_space, logger_path, model_path, cnn_mode):
    ...
    self.dqn = ConvolutionalNeuralNetwork(self.input_shape, action_space, cnn_mode).model
...
class DQNSolver(DQNGameModel):
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    testing_model_path = "./output/neural_nets/" + game_name + "/dqn/model/" + getDate + "/model.h5"
    ...
    DQNGameModel.__init__(self,
                           ...
                           input_shape,
                           action_space,
                           "./output/logs/" + game_name + "/dqn/testing/" + getDate + "/",
                           testing_model_path,
                           cnn_mode)
  ...
  def move(self, state):
    ...
    q_values = self.dqn.predict(np.expand_dims(np.asarray(state).astype(np.float64), axis=0), batch_size=1)
...
class DQNTrainer(DQNGameModel):
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    DQNGameModel.__init__(self,
                           ...
                           input_shape,
                           action_space,
                           "./output/logs/" + game_name + "/dqn/training/" + getDate + "/",
                           "./output/neural_nets/" + game_name + "/dqn/model/" + getDate + "/model.h5",
                           cnn_mode)
    ...
    self.dqn_target = ConvolutionalNeuralNetwork(self.input_shape, action_space, cnn_mode).model
  ...
  def _train(self):
    ...
    for entry in batch:
      ...
      q = list(self.dqn.predict(current_state)[0])
