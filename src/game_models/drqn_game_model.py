...
class DRQNGameModel(BaseGameModel):
  def __init__(self, game_name, mode_name, input_shape, action_space, logger_path, model_path, cnn_mode):
    ...
    self.drqn = RecurrentConvolutionalNeuralNetwork(self.input_shape, action_space, cnn_mode).model
    if os.path.isfile(self.model_path):
      self.drqn.load_weights(self.model_path)

  def _save_model(self):
    self.drqn.save_weights(self.model_path)


class DRQNSolver(DRQNGameModel):
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    testing_model_path = "./output/neural_nets/" + game_name + "/drqn/model/" + getDate + "/model.h5"
    assert os.path.exists(os.path.dirname(testing_model_path)), "No testing model in: " + str(testing_model_path)
    DRQNGameModel.__init__(self,
                           game_name,
                           "DRQN testing",
                           input_shape,
                           action_space,
                           "./output/logs/" + game_name + "/drqn/testing/" + getDate + "/",
                           testing_model_path,
                           cnn_mode)
  ...
  def move(self, state):
    ...
    q_values = self.drqn.predict(np.expand_dims(np.asarray(state).astype(np.float64), axis=0), batch_size=1)
    return np.argmax(q_values[0])


class DRQNTrainer(DRQNGameModel):
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    DRQNGameModel.__init__(self,
                           game_name,
                           "DRQN training",
                           input_shape,
                           action_space,
                           "./output/logs/" + game_name + "/drqn/training/" + getDate + "/",
                           "./output/neural_nets/" + game_name + "/drqn/model/" + getDate + "/model.h5",
                           cnn_mode)
    ...
    self.drqn_target = RecurrentConvolutionalNeuralNetwork(self.input_shape, action_space, cnn_mode).model
  ...
  def _train(self):
    ...
    for entry in batch:
      ...
      next_q_value = np.max(next_state)
      q = list(self.drqn.predict(current_state)[0])
    ...
    fit = self.drqn.fit(np.asarray(current_states).squeeze(),
  ...
  def _reset_target_network(self):
    self.drqn_target.set_weights(self.drqn.get_weights())
