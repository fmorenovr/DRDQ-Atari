...
from convolutional_networks.neural_networks import ConvolutionalNeuralNetwork, RecurrentConvolutionalNeuralNetwork
...
class GEGameModel(BaseGameModel):
  ...
  def __init__(self, game_name, mode_name, input_shape, action_space, logger_path, model_path, cnn_mode):
    ...
    self.model = ConvolutionalNeuralNetwork(input_shape, action_space, cnn_mode).model
    if RNN:
      self.model = RecurrentConvolutionalNeuralNetwork(input_shape, action_space, cnn_mode).model
...
class GESolver(GEGameModel):
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    testing_model_path = "./output/neural_nets/" + game_name + "/ge/model/" + getDate + "/model.h5"
    ...
    GEGameModel.__init__(self,
                         ...
                         input_shape,
                         action_space,
                         "./output/logs/" + game_name + "/ge/testing/" + getDate + "/",
                         testing_model_path,
                         cnn_mode)
...
class GETrainer(GEGameModel):
  ...
  def __init__(self, game_name, input_shape, action_space, getDate, cnn_mode):
    GEGameModel.__init__(self,
                         ...
                         input_shape,
                         action_space,
                         "./output/logs/" + game_name + "/ge/training/"+ getDate + "/",
                         "./output/neural_nets/" + game_name + "/ge/model/" + getDate + "/model.h5",
                         cnn_mode)
  ...
  def _strongest_parents(self, population, env):
    ...
    print("top_min : " + str(min(top_scores)))
    print("top_avg : " + str(mean(top_scores)))
    print("top_max : " + str(max(top_scores)))
