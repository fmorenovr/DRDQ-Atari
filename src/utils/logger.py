...
class Logger:
    ...
    def __init__(self, header, directory_path):
        ...
        self.reward = Stat("update", "reward", TRAINING_UPDATE_FREQUENCY, directory_path, header)
...
class Stat:
    ...
    def add_entry(self, value):
        ...
        if len(self.values) % self.update_frequency == 0:
            ...
            print ("\n"+self.y_label + ": "+ str(value) +" & (min: " + str(min(self.values)) + ", avg: " + str(mean_value) + ", max: " + str(max(self.values))+")")
            #print ('{"metric": "' + self.y_label + '", "value": {}}}'.format(mean_value))
            #print ("\n{" + self.y_label + ": " + str(mean_value)+"}")
